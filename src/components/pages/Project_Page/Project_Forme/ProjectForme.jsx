...
import {
  ...
  getDescricaoByCodigoHabilidade,
} from '../../../../data/bnccHelper.js';

const ProjectForme = forwardRef(({ handleSubmit, projectData, btnText, initialData }, ref) => {
  ...
  const [project, setProject] = useState(() => {
    ...
    if (projectData && Object.keys(projectData).length > 0) {
        return { ...defaults, ...projectData };
    }

    // Restaura rascunho (criação)
  ...
  // --- EFEITOS DE EXPANSÃO AUTOMÁTICA ---
  useEffect(() => {
  ...
  // Preencher formulário em modo de edição (se projectData mudar)
  useEffect(() => {
    if (projectData && Object.keys(projectData).length > 0) {
      setProject(prev => ({
        ...prev,
  ...
  // Preencher formulário com rascunho (se initialData mudar e não estivermos em modo de edição)
  useEffect(() => {
    // Só age se houver rascunho e NÃO estivermos no modo de edição
    if (initialData && Object.keys(initialData).length > 0 && (!projectData || Object.keys(projectData).length === 0)) {
        // Os campos principais já foram mesclados na inicialização,
        // mas caso initialData tenha mudado (raro), atualizamos.
        setProject(prev => ({
            ...prev,
            ...initialData,
            serieAno: initialData.grauId ? String(initialData.grauId) : prev.serieAno,
            categoryId: initialData.categoryId ? Number(initialData.categoryId) : prev.categoryId,
            reviewerComments: initialData.reviewerComments || '',
        }));

        // Restaura alternativas
        if (initialData.alternatives) {
            const lines = initialData.alternatives.split('\n');
            const altObj = {};
            lines.forEach(line => {
                const match = line.match(/^([a-e])\)\s*(.*)$/i);
                if (match) altObj[match[1].toUpperCase()] = match[2].trim();
            });
            if (Object.keys(altObj).length === 5) setAlts(altObj);
        }
    }
}, [initialData, projectData]);  

  // --- EXPOR A FUNÇÃO DE OBTER DADOS ATUAIS (para backup) ---
  ...
  // --- LÓGICA CASCATA: ATUALIZAR TEMAS QUANDO SÉRIE/ANO MUDA ---
  useEffect(() => {
    if (project.serieAno) {
  ...
  // --- LÓGICA CASCATA: ATUALIZAR OBJETOS QUANDO TEMA MUDA ---
  useEffect(() => {
  ...
  // --- LÓGICA CASCATA: ATUALIZAR HABILIDADES QUANDO OBJETO MUDA ---
  useEffect(() => {
...
ProjectForme.displayName = 'ProjectForme';
